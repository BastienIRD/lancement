---
title: "global_catch_firms_level0_analyse"
...
params:
  ...
  overlapping_zone_iattc_wcpfc_data_to_keep: NULL
...
```{r}
nominal_catch <- read.csv(file = here("Data/global_nominal_catch_firms_level0.csv"),stringsAsFactors=TRUE)
dataset <- read.csv("Data/global_catch_firms_level0.csv", stringsAsFactors=TRUE) %>% mutate(gear = as.factor(gear)) %>% mutate(time_start = lubridate::as_date(time_start), time_end = lubridate::as_date(time_end))
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
#mapping_map_code_lists is TRUE on this markdown
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
         ...
         #-----------------------------------------------------------------------------------------------------------------------------------------------------------
           gear_filter<-unique(georef_dataset$gear)
...
Il n'y a pas de filtres sur les engins. Ils sont tous gardés.
...
Tout est gardé dans cette première partie.
...
variables <- colnames(georef_dataset)[colnames(georef_dataset) %notin% c("unit", "value","gear")]
  gear_filtered_grouped_all <- georef_dataset[0,]
